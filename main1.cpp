...
static const char* commandStr[] = {"Init",           "AddCarType",    "RemoveCarType",
                                   "SellCar",        "MakeComplaint", "GetBestSellerModelByType",
                                   "GetWorstModels", "Quit"};
...
{
    if (command == NULL || strlen(command) == 0 || StrCmp("\n", command)) return (NONE_CMD);
    if (StrCmp("#", command)) {
        if (strlen(command) > 1) printf("%s", command);
        return (COMMENT_CMD);
    };
...
static errorType OnAddCarType(void* DS, const char* const command)
{
    ...
    ValidateRead(sscanf_s(command, "%d %d", &typeID, &numOfModels), 2, "%s failed.\n", commandStr[ADDCARTYPE_CMD]);
...
{
    int typeID;
    ValidateRead(sscanf_s(command, "%d", &typeID), 1, "%s failed.\n", commandStr[REMOVECARTYPE_CMD]);
...
{
    int typeID, modelID;
    ValidateRead(sscanf_s(command, "%d %d", &typeID, &modelID), 2, "%s failed.\n", commandStr[SELLCAR_CMD]);
...
static errorType OnMakeComplaint(void* DS, const char* const command)
{
    ...
    ValidateRead(sscanf_s(command, "%d %d %d", &typeID, &modelID, &time), 3, "%s failed.\n",
...
static errorType OnGetWorstModels(void* DS, const char* const command)
{
    ...
    ValidateRead(sscanf_s(command, "%d", &numOfModels), 1, "%s failed.\n", commandStr[GETWORSTMODELS_CMD]);
    if (numOfModels > 0) {
